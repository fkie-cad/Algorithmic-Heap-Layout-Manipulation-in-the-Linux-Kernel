...
class EvoHeap(ABC):
    def __init__(self, types, groups, pmutate=0.9, pmate=0.1, max_mutations=5, max_init_length=15, alloc_free_ratio=0.9, alloc_loop_enabled=False, spray_min=5, spray_max=15, pmutate_classic=0.7, pmutate_spray=0.1, pmutate_hole_spray=0.1, pmutate_shorten=0.1, elitism_frac=0.5, lamb=200, mu=200, target_dist = -96, max_allocs=None, fst_allocs=None, snd_allocs=None, distances=[], population=[]):
        ...
        self._max_mutations = max_mutations #Maximum number of mutations per individual
        self._max_init_length = max_init_length # Maximum length of an individual when it is first created
        self._population = [creator.Individual(ind) for ind in population] # The population. Can either be empty, in which case it will be generated, or can be filled from previous generation
        self._alloc_free_ratio = alloc_free_ratio # Ratio of allocs/free on initialisation of population
    ...
    def _get_freeable_ids(self, ind, position):
        ...
        for i, dir in enumerate(ind):
            ...
            elif self._get_attribute(dir, TYPE_MASK) == FREE_T and self._get_attribute(dir, ID_MASK) in ids:
    ...
    def _mutate(self, ind):
        ...
        for i in range(self._max_mutations):
            ...
            elif r < self._pmutate_classic + self._pmutate_spray + self._pmutate_hole_spray:
    ...
    def _mutate_spray(self, ind):
        ...
        if len(ind) <= 2:
            ...
            location = rnd.randrange(0, len(ind)-1) # Substract 1, as the last one is alway snd
        alloc_primitive = self._get_random_alloc()
        for _ in range(length):
            alloc_primitive = self._set_attribute(alloc_primitive, rnd.randint(0, MAX_ALLOC_ID), ID_MASK)
    ...
    def _select(self, pop):
        ...
        offspring_tournament = tools.selDoubleTournament(pop, int(self._mu * (1 - self._e))  , fitness_size=5, parsimony_size=1.4, fitness_first=True)
    ...
    def _create_directive(self):
        ...
        rep = rnd.randint(1, 0xffff)# Should probably be bounded to maximum elements in cache but lets see
    ...
    def _get_random_alloc(self):
        ...
        assert(self._alloc_loop_enabled == False), "Allocation loops are currently not supported"
    ...
    def get_children(self):
        ...
        while i < self._lambda:
            ...
            if r < self._pmutate:
                ...
                if self._check_length_restriction(new_A) and self._check_length_restriction(new_B):
        ...
        for dir in ind:
            ...
            if type == ALLOC_T:
                ...
                if alloc_id in once_used_ids:
                    candidate.append(f"fengshui{alloc_id} = shmget(IPC_PRIVATE, 1024, IPC_CREAT);")
                else:
                    once_used_ids.append(alloc_id)
                    candidate.append(f"int fengshui{alloc_id} = shmget(IPC_PRIVATE, 1024, IPC_CREAT);")
                algo_id = self._get_attribute(dir, ID_MASK)
                id_mapping[algo_id] = alloc_id
                in_use_ids.append(alloc_id)
            elif type == FREE_T:
                algo_id = self._get_attribute(dir, ID_MASK)
                if algo_id in id_mapping:
                    candidate.append(f"shmctl(fengshui{id_mapping[algo_id]}, IPC_RMID, NULL);")
                    in_use_ids.remove(id_mapping[algo_id])
                    id_mapping.pop(algo_id)
            elif type == FST_T:
                if len(candidate) > 0 and candidate[-1].startswith("shmctl"):
...
def one_noise():
    ...
    evo = EvoHeapWithNoise(None, groups, alloc_free_ratio=0.5, target_dist=-256)
    evo.do_evo_step()
...
def real():
    ...
    evo = EvoHeapTargetNoiseWithReal(None, groups, alloc_free_ratio=0.5, target_dist=-256, max_allocs=16, fst_allocs=5, snd_allocs=1)
    evo.do_evo_step()
